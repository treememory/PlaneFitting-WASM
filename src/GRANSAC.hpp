...
namespace GRANSAC
{
	...
	private:
		...
		std::mt19937 m_RandEngine; // Mersenne twister high quality RNG that support *OpenMP* multi-threading

	public:
		RANSAC(void)
		{
			std::random_device SeedDevice;
			m_RandEngine = std::mt19937(SeedDevice());
		...
		bool Estimate(const std::vector<std::shared_ptr<AbstractParameter> > &Data)
		{
			...
			for (int i = 0; i < m_MaxIterations; ++i)
			{
				...
				std::shuffle(RemainderSamples.begin(), RemainderSamples.end(), m_RandEngine); // To avoid picking the same element more than once
